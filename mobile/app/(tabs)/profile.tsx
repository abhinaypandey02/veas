...
import { GET_CURRENT_USER, GET_PLANETS } from "@/constants/graphql/queries";
...
const SIGN_MEANINGS: Record<string, string> = {
  Aries: "Ignites your path with courage, bold initiative, and a pioneering spirit.",
  Taurus: "Provides you with grounded presence, steadfast trust, and an appreciation for lasting beauty.",
  Gemini: "Fills your approach with curiosity, quick-witted adaptability, and a desire for connection.",
  Cancer: "Centers you in deep nurturing, profound intuition, and receptive feminine energy.",
  Leo: "Inspires you toward joyful leadership, authentic self-expression, and a warm, magnetic presence.",
  Virgo: "Focuses your energy on practical healing, devoted service, and refined discernment.",
  Libra: "Draws you toward cultivating balance, relational harmony, and aesthetic perfection.",
  Scorpio: "Grants you the resilience for deep transformation and an unflinching emotional depth.",
  Sagittarius: "Expands your horizons through a quest for philosophical truth and optimistic exploration.",
  Capricorn: "Steadies you with disciplined ambition, structural mastery, and a respect for time.",
  Aquarius: "Awakens your visionary intellect, urging you toward progressive change and collective innovation.",
  Pisces: "Dissolves boundaries, blessing you with mystical insight, compassion, and spiritual depth.",
};
...
const BIG_THREE_CONFIG = [
  ...
  { key: "rising", label: "Rising", symbol: "↑", meaning: "Your Outward Approach" },
] as const;
...
export default function ProfileScreen() {
  ...
  const [getPlanets, { data: planetsData, loading: planetsLoading }] = useAuthQuery(GET_PLANETS);
  const [updateUserMutation, { loading: updateLoading }] = useAuthMutation(UPDATE_USER);
  ...
  const westernSign = user?.dateOfBirth ? getWesternZodiacSign(user.dateOfBirth) : null;
      ...
      <View style={styles.darkHeader}>
        ...
        {(sunPlanet || moonPlanet || ascendant) && (
          <View style={styles.signRow}>
            ...
            {sunPlanet && <Text style={styles.signValue}>☉ {sunPlanet.sign}</Text>}
            {moonPlanet && <Text style={styles.signValue}>☽ {moonPlanet.sign}</Text>}
              ...
              <>
                ...
                {planets.length > 0 && (
                  <View style={styles.chartContainer}>
                    <BirthChart planets={planets as { name: string; sign: string; house: number }[]} />
                  </View>
                )}
                ...
                {hasBigThree && (
                  <View style={styles.blueprintCard}>
                    <Text style={styles.blueprintTitle}>Your Cosmic Blueprint</Text>
                      ...
                      return (
                        <View key={config.key} style={styles.blueprintItem}>
                          <View style={styles.blueprintSignHeader}>
                            <Text style={styles.blueprintSymbol}>{config.symbol}</Text>
                          ...
                          <View style={styles.blueprintBody}>
                            <Text style={styles.blueprintMeaning}>
                              <Text style={styles.blueprintMeaningLabel}>{config.meaning}{"\n"}</Text>
                              {SIGN_MEANINGS[sign] || ""}
                            </Text>
                          </View>
                        </View>
                ...
                {planets.length > 0 && (
                  <View style={styles.planetsCard}>
                    <Text style={styles.blueprintTitle}>Planetary Positions</Text>
                    {planets.map((planet) => (
                  ...
                  {subLoading ? (
                    ...
                    <View style={styles.subContent}>
                      <View style={{ flexDirection: "row", alignItems: "center" }}>
                        {isPro && <MaterialIcons name="auto-awesome" size={18} color="#b8a06e" style={{ marginRight: 6 }} />}
                  ...
                  >
                    <Text
                      style={[
                        ...
                        genderInput === opt.value && styles.genderOptionTextSelected,
                      ]}
                    >
                      {opt.label}
                    </Text>
